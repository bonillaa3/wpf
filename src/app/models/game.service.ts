...
export class GameService {
    ...
    loginFB() {
        FB.login((response: any) => {
            if (response.authResponse) {
             ...
             FB.api('/me?fields=name,email,picture', (response: any) => {
               console.log(response);
               this.login(response.name, 'password', response.email, response.picture.data.url);
        ...
        }, { scopes: 'email,user_photos,user_posts,read_custom_friendlists'});
    }
